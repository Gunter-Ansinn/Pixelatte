...
public class ImagePreviewTest {
    ...
    private void updateImages(JLabel pLabel, JLabel ioLabel, JLabel diffLabel, JLabel info, JTextArea chunkText, JFrame frame) {
        ...
        mapRes2File("/png_tests/" + name + ".png").ifPresent(file -> {
            try {
                // Load Pixelatte
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                ...
                if (pA != pB) {
                    ...
                    // Match: Draw the pixel dimmed, or just black?
                    // Let's draw the original pixel but 50% transparent to show context
                    // Or just black to highlight errors cleanly.
